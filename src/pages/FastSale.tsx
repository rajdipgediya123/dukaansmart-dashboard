...
const FastSale = () => {
  ...
  const [recentSales, setRecentSales] = useState<{ total: number; method: string; time: string }[]>([]);
  ...
  const completeSale = useCallback(
    (method: string) => {
      ...
      playBeep();
      setRecentSales((prev) => [
        { total: saleTotal, method, time: new Date().toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" }) },
        ...prev.slice(0, 4),
      ]);
      setCart([]);
      setSearch("");
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      // Focus search on typing letters
    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-background">
      ...
      {showSuccess && (
        <div className="absolute inset-0 z-[60] pointer-events-none flex items-center justify-center">
          ...
          <span className="absolute text-3xl font-bold text-green-600">✓ Saved!</span>
        </div>
      )}
        ...
        <div className="flex items-center gap-4">
          <div className="text-right">
            ...
            <p className={cn(
              ...
              total > 0 ? "text-green-600" : "text-muted-foreground"
            )}>
